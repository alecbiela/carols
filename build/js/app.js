window.onload = (() => {
    ...
    document.getElementById('next_button').addEventListener('click', nextButtonClick);
    document.getElementById('prev_button').addEventListener('click', prevButtonClick);
    ...
    const handleGesture = () => {
        ...
        if(Math.abs(dY) < (SWIPE_THRESHOLD * 2)) {
            if(dX > SWIPE_THRESHOLD && (currentSong !== 1)) prevButtonClick(); // swipe left, unless we're on the first song
            else if((dX < -SWIPE_THRESHOLD) && (currentSong !== songs.length)) nextButtonClick(); // swipe right, unless we're on the last song
        }
    };

    // on init, load the songs
    loadSongs();
